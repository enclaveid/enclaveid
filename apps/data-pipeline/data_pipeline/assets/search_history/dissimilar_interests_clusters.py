...
def dissimilar_interests_clusters(
    ...
    )[1:]  # Skip the first one since it's for unclustered interests
    ...
    result = df.with_columns(
        fine_dissimilarity_rank=pl.col("cluster_label").map_dict(
            dict(enumerate(fine_dissimilarity_ranking)), default=-1
        ),
        fine_centroid=pl.col("cluster_label").map_dict(
            dict(enumerate(fine_cluster_centroids)), default=None
        ),
        coarse_dissimilarity_rank=pl.col("merged_cluster_label").map_dict(
            dict(enumerate(coarse_dissimilarity_ranking)), default=-1
        ),
        coarse_centroid=pl.col("merged_cluster_label").map_dict(
            dict(enumerate(coarse_cluster_centroids)), default=None
        ),
    )
