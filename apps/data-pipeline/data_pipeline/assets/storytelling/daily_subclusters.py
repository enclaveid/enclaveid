...
async def daily_causality(
    ...
    sampled_df = full_takeout.filter(
        # Sample max_samples from each day
        pl.int_range(pl.len()).shuffle().over("date") < config.max_samples
        if config.max_samples
        else pl.lit(True)
    )

    # Sort by date and concat date and interests
    # TODO: resolve location coordinates too?
    df = (
        sampled_df.sort(pl.col("date"), pl.col("hour"))
        .with_columns(
            pl.concat_str(
                [
                    pl.col("hour"),
            ...
            ).alias("hour_interests")
        )
        .group_by("date")
        .agg(
            [
                # Create chunks of hour_interests
                pl.col("hour_interests")
                .map_batches(
                    lambda x: [
                        "\n".join(x[i : i + config.chunk_size])
                        for i in range(0, len(x), config.chunk_size)
                    ]
                )
                .alias("day_interests"),
            ]
    ...
    for row in df.to_dicts():
        for chunk in row["day_interests"]:
